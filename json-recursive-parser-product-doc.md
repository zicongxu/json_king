...
- 解析成功后，输入面板会自动隐藏，主视图占满可用宽度；顶部左侧提供“隐藏输入/显示输入”切换按钮，用户可随时重新打开或收起输入面板。
...
- 工具：
  - 在浮层右上方显示“JSON”标签；
  - 提供“复制”“压缩复制”按钮，一键复制当前浮层对应的 JSON：
    - 复制：以带缩进的格式复制；
    - 压缩复制：以紧凑的一行 JSON 复制，便于嵌入日志或参数；
